  ...
  CMD ["nginx", "-g", "daemon off;"]`}
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="bg-yellow-50 border-l-4 border-yellow-500 rounded-lg p-5">
            <h4 className="font-bold text-yellow-700 mb-3">💡 Важливо:</h4>
            <p className="text-sm text-yellow-800 leading-relaxed">
              <code className="bg-yellow-200 px-2 py-1 rounded">VITE_API_BASE=/api</code> встановлюється під час build через ARG/ENV, щоб frontend знав куди робити запити через nginx proxy!
            </p>
          </div>
        </div>
      </div>
  ...
  // Слайд 5 - Multi-stage Build
{
  ...
  {`# ═══════════════════════════════════════════════════
  # Stage 1: BUILD (node:18-alpine ~500MB)
  # ═══════════════════════════════════════════════════
  FROM node:18-alpine as build
  WORKDIR /app
  COPY package*.json ./
